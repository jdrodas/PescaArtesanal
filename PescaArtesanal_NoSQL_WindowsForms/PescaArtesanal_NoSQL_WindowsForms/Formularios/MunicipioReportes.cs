    ...
    public partial class MunicipioReportes : Form
    {
        ...
        private void lbxMunicipios_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lbxMunicipios.DataSource != null)
            {
                ...
                string codigoMunicipio = AccesoDatos.ObtenerIdMunicipio(nombreMunicipio, nombreDepartamento);
                txtCodigoMunicipio.Text = codigoMunicipio;

                //Actualizamos el dataSource del DataGridView de actividades asociadas al municipio
                dgvActividades.DataSource = null;
                int totalActividadesMunicipio = AccesoDatos.ObtenerCantidadActividadesPorMunicipio(nombreMunicipio, nombreDepartamento);

                if (totalActividadesMunicipio == 0)
                    gbxActividades.Visible = false;
                else
                {
                    dgvActividades.DataSource = AccesoDatos.ObtenerTablaActividadesPorMunicipio(nombreMunicipio, nombreDepartamento);
